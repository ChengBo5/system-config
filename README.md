...
```
server-setup/
├── setup.sh                              # 一键部署 (gost + cliproxy + nginx)
├── scripts/
...
```bash
# 一键部署 gost + cliproxy + nginx (需要 sudo)
...
bash server-setup/scripts/install-cliproxy.sh       # 下载 CLIProxyAPIPlus
sudo bash server-setup/scripts/start-cliproxy.sh    # 注册 cliproxy 服务
...
# 删除 CLIProxyAPIPlus
sudo systemctl stop cliproxy && sudo systemctl disable cliproxy
sudo rm /etc/systemd/system/cliproxy.service && sudo systemctl daemon-reload
